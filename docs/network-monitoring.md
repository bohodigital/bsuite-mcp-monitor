...
The setup and dashboard examples use documentation-range IP addresses,
placeholder key fingerprints, and synthetic event data. They are generated by
`python docs/generate_screenshots.py` so they stay aligned with the terminal
renderer without publishing host-specific information.

![B-Suite SSH monitoring configuration](screenshots/ssh-configuration.svg)

![B-Suite SSH dashboard with baseline, trend, audit, and alert state](screenshots/ssh-dashboard.svg)
